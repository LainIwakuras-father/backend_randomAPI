...
class RandomService:
    ...
    async def choice_winner(
        ...
        if criteria.likes and criteria.reposts and criteria.comments:
            ...
            comments = await api_VK_client.get_user_ids_by_comment(
                group_id,
                item_id,
            )
            ...
            raise HTTPException(
                status_code=422,
                detail="Неверная комбинация критериев. Выберите хотя бы один критерий.",
            )
        ...
        if required_group:
            try:
                await asyncio.sleep(0.4) #задержка для бота вконтакте чтобы не превысить лимит запросов
                ...
                logging.info(f'{list_groups_ids}')
